...
with col_list:
    ...
    # Nagłówek tabeli
    st.markdown("""
    <div style="display:grid;grid-template-columns:28px 1fr 1fr 70px 90px;gap:4px;
      ...
      <div></div><div>Szczyt</div><div>Notatki</div><div style="text-align:right">Wys.</div><div style="text-align:right">Data</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    for s in disp:
        ...
        notes_s = f'<span style="font-size:0.78rem;color:#6a9ab8;font-style:italic;">💬 {s["notes"]}</span>' if s.get("notes") else '<span style="color:#1e3a58;font-size:0.75rem;">—</span>'
        bg = "#0a1e0e" if done else "#0a1828"
        border = "#1a4a22" if done else "#1a2e42"
        st.markdown(f"""
        <div style="display:grid;grid-template-columns:28px 1fr 1fr 70px 90px;gap:4px;align-items:center;
