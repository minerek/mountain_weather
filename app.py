...
SZCZYTY = {
    ...
    "Pośredni Granat":                               (49.226066, 20.0331, 2234, "Tatry Polskie"),
    ...
    "Kasprowy Wierch (Kasprový vrch)":               (49.2324, 19.9817, 1987, "Tatry Zachodnie"),
    ...
    "Lodowy Szczyt (Ľadový štít) ⭐WKT":          (49.198545, 20.182765, 2627, "Tatry Słowackie"),
...
#   przez     — lista mijanych miejsc
#   czas      — czas tam i z powrotem (str)
#   znaki     — kolor/oznaczenie szlaku
#   trudnosc  — stopień trudności (str)
...
_ROUTE_PHRASES_EN = {
    ...
    # szlaki nazwane
    "czerwony (orla perć)":  "red — Eagle's Path",
    "żółty → czerwony (orla perć)": "yellow → red — Eagle's Path",
    "niebieski → żółty → czerwony (orla perć)": "blue → yellow → red — Eagle's Path",
    "Orla Percią (czerwony)":  "Eagle's Path (red trail)",
    ...
    "strome podejście rynną z dużą ekspozycją": "steep gully ascent with high exposure",
    "strome kominki i eksponowana grań":    "steep chimneys and exposed ridge",
    "głównie strome trawki, głazy i perć":  "mainly steep grass, boulders & scramble",
    "końcowe podejście po wygładzonych skałach": "final approach on polished rock",
    "niebezpieczeństwo spadających kamieni": "rockfall danger",
    "pod Przełęczą Wagą":                   "below Váha Pass",
    "długa pętla kondycyjna, łącznie 20,9 km, pod Polskim Grzebieniem kilka łańcuchów": "long endurance loop, 20.9 km total, few chains below Polski Grzebień pass",
    "klamry i łańcuchy":                    "pegs & chains",
    "miejscami kruchy i stromy żleb":       "partially loose and steep gully",
    "szlak zielony na przełęcz ma trudności 0+ z dużą ekspozycją i kilkoma klamrami": "green trail to pass is 0+ with high exposure and a few pegs",
    ...
    "Powrót przez Rohatka → Zbojnicka Chata → Dolina Staroleśna → Hrebienok → Stary Smokovec": "Return via Rohatka → Zbojnicka Chata → Staroleśna Valley → Hrebienok → Stary Smokovec",
...
with col_pasma:
    wybrane_pasma = st.multiselect(
        ...
        default=["Tatry Polskie", "Tatry Słowackie", "Tatry Zachodnie"],
        help=L["range_help"]
    )
